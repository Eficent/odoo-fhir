...
class ImmunizationRecommendation(models.Model):
    ...
    identifier_ids = fields.One2many(comodel_name="hc.immunization.rec.identifier",
                                     inverse_name="immunization_recommendation_id", string="Identifiers",
    ...
    recommendation_ids = fields.One2many(comodel_name="hc.immunization.rec.rec",
                                         inverse_name="immunization_recommendation_id", string="Recommendations",
                                         required="True", help="Vaccine administration recommendations.")
...
class ImmunizationRecRec(models.Model):
    ...
    immunization_recommendation_id = fields.Many2one(comodel_name="hc.res.immunization.recommendation",
    ...
    vaccine_code_id = fields.Many2one(comodel_name="hc.vs.vaccine.code", string="Vaccine Code", required="True",
                                      help="Vaccine recommendation applies to")
    dose_number = fields.Integer(string="Dose Number", help="Recommended dose number.")
    forecast_status_id = fields.Many2one(comodel_name="hc.vs.immunization.rec.status", string="Forecast Status",
                                         required="True", help="Vaccine administration status.")
    identifier_ids = fields.One2many(comodel_name="hc.immunization.rec.rec.identifier",
                                     inverse_name="immunization_rec_rec_id", string="Identifiers",
    ...
    supporting_patient_info_name = fields.Text(string="Supporting Patient Info",
                                               compute="_compute_supporting_patient_info_name", store="True",
                                               help="Patient observations supporting recommendation.")
    supporting_patient_info_observation_id = fields.Many2one(comodel_name="hc.res.observation",
                                                             string="Supporting Patient Info Observation",
                                                             help="Observation associated with this recommendation.")
    supporting_patient_info_allergy_intolerance_id = fields.Many2one(comodel_name="hc.res.allergy.intolerance",
                                                                     string="Supporting Patient Info Allergy Intolerance",
                                                                     help="Allergy Intolerance associated with this recommendation.")
    date_criterion_ids = fields.One2many(comodel_name="hc.immunization.rec.rec.date.criterion",
                                         inverse_name="immunization_rec_rec_id", string="Date Criterions",
                                         help="Dates governing proposed immunization.")
    protocol_ids = fields.One2many(comodel_name="hc.immunization.rec.rec.protocol",
                                   inverse_name="immunization_rec_rec_id", string="Protocols",
...
class ImmunizationRecRecDateCriterion(models.Model):
    ...
    immunization_rec_rec_id = fields.Many2one(comodel_name="hc.immunization.rec.rec",
                                              string="Immunization Recommendation Recommendation",
                                              help="Recommendation associated with this date criterion.")
    code_id = fields.Many2one(comodel_name="hc.vs.immunization.rec.date.criterion", string="Code", required="True",
                              help="Type of date.")
...
class ImmunizationRecRecProtocol(models.Model):
    ...
    authority_id = fields.Many2one(comodel_name="hc.res.organization", string="Authority",
...
class ImmunizationRecRecIdentifier(models.Model):
    ...
    immunization_rec_rec_id = fields.Many2one(comodel_name="hc.immunization.rec.rec",
                                              string="Immunization Recommendation Recommendation",
                                              help="Recommendation associated with this immunization Recommendation Recommendation identifier.")
