...
class CommunicationRequest(models.Model):
    ...
    identifier_ids = fields.One2many(comodel_name="hc.communication.request.identifier",
                                     inverse_name="communication_request_id", string="Identifiers",
                                     help="Unique identifier.")
    category_id = fields.Many2one(comodel_name="hc.vs.communication.request.category", string="Category",
    ...
    sender_device_id = fields.Many2one(comodel_name="hc.res.device", string="Sender Device",
                                       help="Device message sender.")
    sender_organization_id = fields.Many2one(comodel_name="hc.res.organization", string="Sender Organization",
                                             help="Organization message sender.")
    sender_patient_id = fields.Many2one(comodel_name="hc.res.patient", string="Sender Patient",
                                        help="Patient message sender.")
    sender_practitioner_id = fields.Many2one(comodel_name="hc.res.practitioner", string="Sender Practitioner",
                                             help="Practitioner message sender.")
    sender_related_person_id = fields.Many2one(comodel_name="hc.res.related.person", string="Sender Related Person",
                                               help="Related Person message sender.")
    recipient_ids = fields.One2many(comodel_name="hc.communication.request.recipient",
                                    inverse_name="communication_request_id", string="Recipients",
                                    help="Message recipient.")
    medium_ids = fields.Many2many(comodel_name="hc.vs.participation.mode", string="Mediums",
    ...
    requester_name = fields.Char(string="Requester", compute="_compute_requester_name", store="True",
                                 help="An individual who requested a communication.")
    requester_practitioner_id = fields.Many2one(comodel_name="hc.res.practitioner", string="Requester Practitioner",
                                                help="Practitioner who requested a communication.")
    requester_patient_id = fields.Many2one(comodel_name="hc.res.patient", string="Requester Patient",
                                           help="Patient who requested a communication.")
    requester_related_person_id = fields.Many2one(comodel_name="hc.res.related.person",
                                                  string="Requester Related Person",
                                                  help="Related Person who requested a communication.")
    status = fields.Selection(string="Communication Request Status",
                              selection=[("proposed", "Proposed"), ("planned", "Planned"), ("requested", "Requested"),
    ...
    encounter_id = fields.Many2one(comodel_name="hc.res.encounter", string="Encounter",
    ...
    scheduled_name = fields.Char(string="Scheduled", compute="_compute_scheduled_name", store="True",
                                 help="When scheduled.")
    ...
    payload_ids = fields.One2many(comodel_name="hc.communication.request.payload",
                                  inverse_name="communication_request_id", string="Payloads", help="Message payload.")
...
class CommunicationRequestPayload(models.Model):
    ...
    communication_request_id = fields.Many2one(comodel_name="hc.res.communication.request",
    ...
    content_name = fields.Char(string="Content", compute="_compute_content_name", store="True",
    ...
    content_code_id = fields.Many2one(comodel_name="hc.vs.communication.content.code", string="Content Code",
...
class CommunicationRequestRecipient(models.Model):
    ...
    recipient_type = fields.Selection(
        ...
        selection=[
            ...
            ("Care Team", "Care Team")],
        help="Type of message recipient.")
    recipient_name = fields.Char(string="Recipient", compute="_compute_recipient_name", store="True",
                                 help="Message recipient.")
    recipient_device_id = fields.Many2one(comodel_name="hc.res.device", string="Recipient Device",
                                          help="Device message recipient.")
    recipient_organization_id = fields.Many2one(comodel_name="hc.res.organization", string="Recipient Organization",
                                                help="Organization message recipient.")
    recipient_patient_id = fields.Many2one(comodel_name="hc.res.patient", string="Recipient Patient",
                                           help="Patient message recipient.")
    recipient_practitioner_id = fields.Many2one(comodel_name="hc.res.practitioner", string="Recipient Practitioner",
                                                help="Practitioner message recipient.")
    recipient_related_person_id = fields.Many2one(comodel_name="hc.res.related.person",
                                                  string="Recipient Related Person",
                                                  help="Related Person message recipient.")
    recipient_group_id = fields.Many2one(comodel_name="hc.res.group", string="Recipient Group",
                                         help="Group message recipient.")
    recipient_care_team_id = fields.Many2one(comodel_name="hc.res.care.team", string="Recipient Care Team",
