...
class ProcessResponse(models.Model):
    ...
    identifier_ids = fields.One2many(comodel_name="hc.process.response.identifier", inverse_name="process_response_id",
                                     string="Identifiers", help="Business Identifier.")
    status = fields.Selection(string="Process Response Status",
                              selection=[("active", "Active"), ("cancelled", "Cancelled"), ("draft", "Draft"),
    ...
    organization_id = fields.Many2one(comodel_name="hc.res.organization", string="Organization",
                                      help="Authoring Organization.")
    request_type = fields.Selection(string="Request Type", selection=[("string", "String"), ("code", "Code")],
                                    help="Type of request reference.")
    request_name = fields.Char(string="Request", compute="_compute_request_name", store="True",
                               help="Request reference.")
    request_string = fields.Char(string="Request String", help="String of request reference.")
    request_code_id = fields.Many2one(comodel_name="hc.vs.resource.type", string="Request Code",
    ...
    request_provider_id = fields.Many2one(comodel_name="hc.res.practitioner", string="Request Provider",
                                          help="Responsible Practitioner.")
    request_organization_id = fields.Many2one(comodel_name="hc.res.organization", string="Request Organization",
                                              help="Responsible organization.")
    form_id = fields.Many2one(comodel_name="hc.vs.forms", string="Form", help="Printed Form Identifier.")
    error_ids = fields.Many2many(comodel_name="hc.vs.adjudication.error", relation="process_response_error_rel",
                                 string="Errors", help="Error code.")
    communication_request_ids = fields.One2many(comodel_name="hc.process.response.communication.request",
                                                inverse_name="process_response_id", string="Communication Requests",
                                                help="Request for additional information.")
    process_note_ids = fields.One2many(comodel_name="hc.process.response.process.note",
                                       inverse_name="process_response_id", string="Process Notes",
...
class ProcessResponseProcessNote(models.Model):
    ...
    process_response_id = fields.Many2one(comodel_name="hc.res.process.response", string="Process Response",
                                          help="Process Response associated with this Process Response Process Note.")
    type = fields.Selection(string="Type",
                            selection=[("display", "Display"), ("print", "Print"), ("printoper", "Printoper")],
